...
/**
 * Runs the SQLX self-update for the configured executable.
 */
@JcefAction(value = "update-sqlx", method = "client-command")
public class UpdateSqlxHandler implements IJcefActionHandler {
    ...
    public void handle(ConsoleMessage consoleMessage, ConsoleResult wsResult, CefQueryCallback callback) {
        try {
            ...
            ResponseBuilder.buildSuccessJcef(
                    Map.of("data", SqlxBridgeRegistry.getBridge().update(operationId)), callback);
