...
if [ "$(jq -r '.metadataSha256' "${TEST_ROOT}/artifact/latest_version.json")" != "$(shasum -a 256 "${TEST_ROOT}/artifact/version.json" | awk '{print $1}')" ]; then
  fail "latest_version.json metadata SHA mismatch"
fi
