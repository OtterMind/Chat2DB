...
assert.deepEqual(activity(calls), { kind: 'tool', names: ['db_query', 'read'] });
assert.deepEqual(activity([...calls, done(3, 'second')]), { kind: 'tool', names: ['db_query'] });
...
assert.deepEqual(getAgentActivity(true, calls, 'run', [{ ...question, status: 'answered' }], []), { kind: 'tool', names: ['db_query', 'read'] });
...
const live = appendAgentTimeline([], [{ id: 'start', sessionId: 'session', runId: 'run', sequence: 1,
  type: 'TOOL_CALL_RUNNING', payload: { toolCallId: 'call', toolName: 'read' }, occurredAt: '' }]);
assert.deepEqual(activity(live), { kind: 'tool', names: ['read'] });
