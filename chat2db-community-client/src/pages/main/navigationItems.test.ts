...
const items = createCoreMainNavItems({
  ...
  sessions: { component: 'sessions-component', name: 'Sessions' },
...
assert.deepEqual(
  ...
  CORE_MAIN_NAV_KEYS,
  'shared main navigation should keep the Stream, Workspace, Sessions, Dashboard order',
);
...
assert.strictEqual(items[2].icon, Activity, 'Sessions should use the semantic Lucide icon');
...
assert.match(communityMainPage, /<SessionMonitor \/>/);
